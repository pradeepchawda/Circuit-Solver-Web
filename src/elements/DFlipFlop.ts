...
or overlapped)*/
  ...
  update() {
    if (global.FLAG_SIMULATING && simulation_manager.SOLUTIONS_READY) {
      if (this.elm.consistent()) {
        this.elm.properties['Last Clock'] = global.copy(
          this.elm.properties['Clock']
        );
        this.elm.properties['Input Voltage1'] = Math.tanh(
          10 * (engine_functions.get_voltage(this.elm.n1, -1) - 0.5)
        );
        this.elm.properties['Clock'] = Math.tanh(
          10 * (engine_functions.get_voltage(this.elm.n2, -1) - 0.5)
        );
        if (
          Math.abs(
            this.elm.properties['Last Clock'] - this.elm.properties['Clock']
          ) > 0.5 &&
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_voltage(
        this.elm.n3,
        -1,
        this.elm.properties['Q'],
        simulation_manager.ELEMENT_DFF_OFFSET + (this.simulation_id << 1)
      );
      engine_functions.stamp_voltage(
        this.elm.n4,
        -1,
        this.elm.properties['!Q'],
        simulation_manager.ELEMENT_DFF_OFFSET + (this.simulation_id << 1) + 1
      );
    }
  }
