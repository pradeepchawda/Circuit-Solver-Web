...
class VoltageControlledVoltageSource {
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_vcvs(
        this.elm.n1,
        this.elm.n2,
        this.elm.n3,
        this.elm.n4,
        -this.elm.properties['Gain'],
        simulation_manager.ELEMENT_VCVS_OFFSET + this.simulation_id
      );
    }
  }
  ...
  draw_component(canvas) {
    ...
    if (
      ...
      /* Reference polarity point */
      this.LINE_BUFFER[this.indexer++] = Array(
        this.vcvs_16.x - this.bounds.get_width() * 0.05,
        this.vcvs_16.y,
        this.vcvs_16.x + this.bounds.get_width() * 0.05,
        this.vcvs_16.y
      );
      this.LINE_BUFFER[this.indexer++] = Array(
        this.vcvs_16.x,
        this.vcvs_16.y + this.bounds.get_width() * 0.05,
        this.vcvs_16.x,
        this.vcvs_16.y - this.bounds.get_width() * 0.05
      );
        ...
        if (
          ...
          canvas.draw_text(
            global.ELEMENT_VAL_TEMPLATE.replace(
              '{VAL}',
              global.exponentiate_quickly(this.elm.properties['Gain'])
