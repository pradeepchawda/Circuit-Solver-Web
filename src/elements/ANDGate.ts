	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['Output Voltage'] = (this.elm.properties['High Voltage'] * 2.0) / (2 / (1 + this.elm.properties['Input Voltage1'] + global.CONSTANTS.ZERO_BIAS) + 2.0 / (1 + this.elm.properties['Input Voltage2'] + global.CONSTANTS.ZERO_BIAS));
			}
		}
	}
	stamp(): void {
