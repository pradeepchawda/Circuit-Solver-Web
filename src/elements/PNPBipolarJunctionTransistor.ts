...
class PNPBipolarJunctionTransistor {
  ...
  public pnp_2 = new PointF(0, 0);
  public pnp_3 = new PointF(0, 0);
  public pnp_4 = new PointF(0, 0);
  public pnp_5 = new PointF(0, 0);
  public pnp_6 = new PointF(0, 0);
  public pnp_7 = new PointF(0, 0);
  public pnp_8 = new PointF(0, 0);
...
or overlapped)*/
  ...
  stamp() {
    if (this.elm.consistent()) {
      ...
      engine_functions.stamp_across_nodes(
        c,
        b,
        -1.0 / this.elm.properties['g_ce']
      );
      engine_functions.stamp_across_nodes(
        b,
        e,
  ...
  get_pnpbjt_error() {
    return Math.abs(
      this.elm.properties['Veb'] - this.elm.properties['Last Veb']
    );
  }
    ...
    if (global.FLAG_SIMULATING && simulation_manager.SOLUTIONS_READY) {
      if (this.elm.consistent()) {
        ...
        if (next_veb > this.DAMPING_SAFETY_FACTOR * vcrit) {
          veb = global.log_damping(
            next_veb,
            this.elm.properties['Veb'],
            this.GAMMA,
            this.KAPPA
          );
        ...
        if (next_vcb > this.DAMPING_SAFETY_FACTOR * vcrit) {
          vcb = global.log_damping(
            next_vcb,
            this.elm.properties['Vcb'],
            this.GAMMA,
            this.KAPPA
          );
        ...
        /* Update the pnpbjt */
        let forward_alpha =
          this.elm.properties['Forward Beta'] /
          (1 + this.elm.properties['Forward Beta']);
        let reverse_alpha =
          this.elm.properties['Reverse Beta'] /
  ...
  gmin_step(step, error) {
    ...
    if (
      simulation_manager.ITERATOR > step &&
        ...
        if (
          ...
          canvas.draw_text(
            global.ELEMENT_VAL_TEMPLATE.replace(
              '{VAL}',
              global.exponentiate_quickly(this.elm.properties['Forward Beta'])
