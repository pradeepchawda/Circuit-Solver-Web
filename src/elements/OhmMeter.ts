 ...
 *
 * Purpose           : A class to handle the ohmmeter element. It will automatically generate
...
or overlapped)*/
  ...
  constructor(type, id, n1, n2) {
    ...
    /* Inititalize the element2 class that will hold the basic data about our component */
    this.elm = new Element2(id, type, global.copy(global.PROPERTY_OHMMETER));
    ...
    if (this.elm.consistent()) {
      ...
      this.trace_bounds.set_bounds(
        this.c_x - global.node_space_x,
        this.c_y - 2 * global.node_space_y,
        this.c_x + global.node_space_x,
        this.c_y - 1 * global.node_space_y
      );
      this.meter_trace.update_parameters(
        this.trace_bounds,
        this.RATIO,
        this.trace_bounds.get_width(),
        this.trace_bounds.get_height(),
        0
      );
    }
    ...
    this.meter_symbol.reset(
      this.meter_symbol.METER_RESISTANCE,
      this.meter_symbol.STYLE_0
    );
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_voltage(
        this.elm.n1,
        this.elm.n2,
        this.elm.properties['Test Voltage'],
        simulation_manager.ELEMENT_OHMMETER_OFFSET + this.simulation_id
      );
    }
  }
  ...
  build_element() {
    if (this.BUILD_ELEMENT || global.SIGNAL_BUILD_ELEMENT) {
      ...
      this.meter_symbol.set_bounds(
        this.bounds.left + w_cache,
        this.bounds.top + h_cache,
        this.bounds.right - w_cache,
        this.bounds.bottom - h_cache
      );
  ...
  push_voltage_current(voltage, current) {
    if (
      ...
      this.meter_trace.push(
        this.elm.properties['Sensed Resistance'],
        global.SIMULATION_TIME
      );
    }
  }
  ...
  recolor() {
    ...
    this.SCOPE_INDEX_CHECK = scope_manager.find_entry_index(
      this.elm.id,
      this.elm.type
    );
