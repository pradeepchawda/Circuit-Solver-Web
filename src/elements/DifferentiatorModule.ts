...
or overlapped)*/
  ...
  constructor(type, id, n1, n2) {
    ...
    /* Inititalize the element2 class that will hold the basic data about our component */
    this.elm = new Element2(
      id,
      type,
      global.copy(global.PROPERTY_DIFFERENTIATOR)
    );
    ...
    if (
      ...
      if (this.elm.consistent()) {
        if (global.SIMULATION_TIME < global.TIME_STEP * 1.5) {
          this.elm.properties['Last Value'] = global.copy(
            this.elm.properties['Initial Value']
          );
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_voltage(
        ...
        -1,
        this.elm.properties['Output Voltage'],
        simulation_manager.ELEMENT_DIFFERENTIATOR_OFFSET + this.simulation_id
      );
    }
  }
