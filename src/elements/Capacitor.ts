...
or overlapped)*/
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_capacitor(
        this.elm.n1,
        this.elm.n2,
  ...
  build_element() {
    if (this.BUILD_ELEMENT || global.SIGNAL_BUILD_ELEMENT) {
      ...
      this.cap_2.x =
        this.c_x +
        cache_0 * global.cosine(this.theta) +
        cache_1 * global.cosine(Math.PI + this.theta_m90);
      this.cap_2.y =
        this.c_y +
        cache_2 * global.sine(this.theta) +
        cache_3 * global.sine(Math.PI + this.theta_m90);
      this.cap_3.x =
        this.c_x -
        cache_0 * global.cosine(this.theta) +
        cache_1 * global.cosine(Math.PI + this.theta_m90);
  ...
  resize() {
    if (this.BUILD_ELEMENT || global.SIGNAL_BUILD_ELEMENT) {
      if (this.bounds.anchored) {
        ...
        /* This is to help wires when resizing events occur. */
        this.unanchor_wires();
        this.anchor_wires();
      } else {
        this.refactor();
  ...
  reset_capacitor() {
    ...
    this.elm.properties['Transient Voltage'] = global.copy(
      this.elm.properties['Initial Voltage']
    );
        ...
        if (
          ...
          canvas.draw_text(
            global.ELEMENT_VAL_TEMPLATE.replace(
              '{VAL}',
              global.exponentiate_quickly(this.elm.properties['Capacitance'])
