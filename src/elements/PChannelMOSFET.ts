...
class PChannelMOSFET {
  ...
  get_pmosfet_error() {
    return Math.abs(
      this.elm.properties['Vsg'] - this.elm.properties['Last Vsg']
    );
  }
  ...
  update() {
    if (global.FLAG_SIMULATING && simulation_manager.SOLUTIONS_READY) {
      if (this.elm.consistent()) {
        this.elm.properties['Last Vsg'] = global.copy(
          this.elm.properties['Vsg']
        );
        this.elm.properties['Last Io'] = global.copy(this.elm.properties['Io']);
        this.elm.properties['Vsg'] = global.log_damping(
          engine_functions.get_voltage(this.elm.n1, this.elm.n3),
          this.elm.properties['Vsg'],
          this.GAMMA,
          this.KAPPA
        );
        this.elm.properties['Vsd'] = global.log_damping(
        ...
        let kp =
          0.5 * this.elm.properties['W/L Ratio'] * -this.elm.properties["K'p"];
