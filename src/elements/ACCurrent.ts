	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_current(
				this.elm.n1,
				this.elm.n2,
				global.utils.sine(2 * Math.PI * this.elm.properties['Frequency'] * global.simulation_time + global.to_radians(this.elm.properties['Phase'])) * this.elm.properties['Current'] +
		...
		if (
			...
			if (global.variables.workspace_zoom_scale > 1.085 || (!global.CONSTANTS.MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
				...
				if ((this.angle > 170 && this.angle < 190) || (this.angle > -10 && this.angle < 10)) {
					canvas.draw_text(
						global.ELEMENT_VAL_TEMPLATE.replace('{VAL}', global.utils.exponentiate_quickly(this.elm.properties['Current'])).replace('{UNIT}', this.elm.properties['units']),
						this.c_x,
