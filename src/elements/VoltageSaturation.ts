...
or overlapped)*/
  ...
  update() {
    if (
      ...
      if (this.elm.consistent()) {
        this.elm.properties['Input Voltage'] = engine_functions.get_voltage(
          this.elm.n1,
          -1
        );
        this.elm.properties['Output Voltage'] = global.limit(
  ...
  stamp() {
    if (this.elm.consistent()) {
      engine_functions.stamp_voltage(
        ...
        simulation_manager.ELEMENT_VSAT_OFFSET + this.simulation_id
      );
    }
  }
  ...
  map_rotation() {
    if (
      ...
      this.elm.rotation === global.ROTATION_90 ||
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          canvas.draw_text(
            global.DIVISION_TEXT_TEMPLATE.replace(
              '{A}',
              global.exponentiate_quickly(this.elm.properties['Low Voltage'])
            ).replace(
              '{B}',
