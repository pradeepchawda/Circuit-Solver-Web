...
class ADCModule {
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY_ADC));
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['Output Voltage'] = global.utils.limit(global.cast_int(this.elm.properties['Input Voltage'] / this.elm.properties['LSB']), 0, this.elm.properties['Max Bits']);
			}
		}
	}
	stamp(): void {
