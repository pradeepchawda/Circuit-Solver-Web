...
class Relay {
  ...
  public relay_2 = new PointF(0, 0);
  public relay_3 = new PointF(0, 0);
  public relay_4 = new PointF(0, 0);
  public relay_5 = new PointF(0, 0);
  public relay_6 = new PointF(0, 0);
  public relay_7 = new PointF(0, 0);
  public relay_8 = new PointF(0, 0);
  public relay_9 = new PointF(0, 0);
  public relay_10 = new PointF(0, 0);
  public relay_11 = new PointF(0, 0);
...
or overlapped)*/
  ...
  reset_relay() {
    ...
    this.elm.properties['Transient Current'] = global.copy(
      this.elm.properties['Initial Current']
    );
    this.elm.properties['Equivalent Current'] =
      this.elm.properties['Transient Voltage'] /
        this.elm.properties['Transient Resistance'] +
  ...
  stamp() {
    if (this.elm.consistent()) {
      ...
      if (
        ...
        engine_functions.stamp_resistor(
          this.elm.n3,
          this.elm.n4,
          this.elm.properties['Closed Resistance']
        );
      } else {
        engine_functions.stamp_node(
          this.elm.n3,
          this.elm.properties['Open Resistance']
        );
        engine_functions.stamp_node(
          this.elm.n4,
          this.elm.properties['Open Resistance']
      ...
      if (
        ...
        if (
          this.bounds.contains_xywh(
            global.mouse_x,
            global.mouse_y,
            this.bounds.get_width() >> 1,
            this.bounds.get_height() >> 1
          ) &&
          !global.component_touched
        ) {
          this.is_translating = false;
          ...
          if (
            ...
            if (
              nodes[this.elm.n1].contains_xy(global.mouse_x, global.mouse_y)
            ) {
              this.handle_wire_builder(this.elm.n1, global.ANCHOR_POINT['p1']);
              global.component_touched = true;
            } else if (
              nodes[this.elm.n2].contains_xy(global.mouse_x, global.mouse_y)
            ) {
              this.handle_wire_builder(this.elm.n2, global.ANCHOR_POINT['p2']);
              global.component_touched = true;
            } else if (
              nodes[this.elm.n3].contains_xy(global.mouse_x, global.mouse_y)
            ) {
              this.handle_wire_builder(this.elm.n3, global.ANCHOR_POINT['p3']);
              global.component_touched = true;
            } else if (
              nodes[this.elm.n4].contains_xy(global.mouse_x, global.mouse_y)
            ) {
      ...
      for (var i = this.wire_reference.length - 1; i > -1; i--) {
        ...
        if (id > -1 && id < wires.length) {
          if (
            ...
            this.wire_reference[i]['anchor_point'] === global.ANCHOR_POINT['p3']
          ) {
            wires[id].release_nodes();
            if (this.wire_reference[i]['linkage'] === 0) {
            ...
            this.wire_reference[i]['anchor_point'] === global.ANCHOR_POINT['p4']
          ) {
            wires[id].release_nodes();
            if (this.wire_reference[i]['linkage'] === 0) {
  ...
  draw_component(canvas) {
    ...
    if (
      ...
      this.CIRCLE_BUFFER[this.indexer++] = Array(
        this.p1.x,
        this.p1.y,
      ...
      if (
        ...
        this.ANGLE = global.retrieve_angle(
          this.p2.x - this.p1.x,
          this.p2.y - this.p1.y
        );
        if (
          (this.ANGLE > 170 && this.ANGLE < 190) ||
          ...
          canvas.draw_text(
            global.ELEMENT_VAL_TEMPLATE.replace(
              '{VAL}',
              global.exponentiate_quickly(this.elm.properties['Inductance'])
