...
or overlapped)*/
  ...
  get_nmosfet_error() {
    return Math.abs(
      this.elm.properties['Vgs'] - this.elm.properties['Last Vgs']
    );
  }
  ...
  update() {
    if (global.FLAG_SIMULATING && simulation_manager.SOLUTIONS_READY) {
      if (this.elm.consistent()) {
        ...
        this.elm.properties['Vgs'] = global.log_damping(
          engine_functions.get_voltage(this.elm.n3, this.elm.n2),
          this.elm.properties['Vgs'],
          this.GAMMA,
          this.KAPPA
        );
        this.elm.properties['Vds'] = global.log_damping(
          engine_functions.get_voltage(this.elm.n1, this.elm.n2),
        ...
        let kn =
          0.5 * this.elm.properties['W/L Ratio'] * this.elm.properties["K'n"];
        ...
        if (
          ...
          canvas.draw_text(
            global.ELEMENT_VAL_TEMPLATE.replace(
              '{VAL}',
              global.exponentiate_quickly(this.elm.properties['W/L Ratio'])
