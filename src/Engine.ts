...
function load_app() {
  ...
  function resize_canvas() {
    ...
    solver_container.style.width = global.PIXEL_TEMPLATE.replace(
      '{VALUE}',
      String(window.innerWidth)
    );
    solver_container.style.height = global.PIXEL_TEMPLATE.replace(
      '{VALUE}',
    ...
    if (global.SYSTEM_INITIALIZATION['completed']) {
      ...
      if (bottom_menu.handle_file_explorer()) {
        if (!global.USER_FILE_SELECTED) {
          ...
          toast.set_text(
            language_manager.TRY_AGAIN[global.LANGUAGES[global.LANGUAGE_INDEX]]
          );
          toast.show();
        }
      } else {
  ...
  function system_loop() {
    try {
      ...
      if (global.CANVAS_DRAW_EVENT) {
        ...
        FPS_DIV ^= 1;
        if (
          ((FPS_DIV == 1 || TEMP_DRAW_SIGNAL) && global.FLAG_SIMULATING) ||
          !global.FLAG_SIMULATING
        ) {
          if (global.SYSTEM_INITIALIZATION['completed']) {
            if (
              (global.FLAG_SIMULATING && global.CANVAS_DRAW_REQUEST) ||
              TEMP_DRAW_SIGNAL
            ) {
      ...
      if (!global.MOBILE_MODE) {
        ...
        if (global.WIRE_BUILDER['step'] > 0) {
          ...
          if (
            ...
            canvas.draw_line_buffer(
              global.NODE_LINE_BUFFER,
              nodes[global.WIRE_BUILDER['n1']].node_line_paint
            );
            canvas.draw_rect2(
