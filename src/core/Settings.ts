...
class Settings {
  ...
  public WORKSPACE_PERFECT_SQUARE: boolean = true;
  /* The Workspace Scaling (ultimately determines width and height of workspace view) */
  public WORKSPACE_RATIO_Y: number = 0.45;
  public WORKSPACE_RATIO_X: number = 0;
  /* Resistance for near zero voltage drop elements, like ammeters. */
  public WIRE_RESISTANCE: number = 1e-3;
  /* This is the multiplier for dx and dy when dragging the workspace's view */
  public TRANSLATION_SCALE: number = 1.2;
  /* The maximum number of nodes to create for the given board. Must be a perfect square.*/
  public MAXNODES: number = 900;
  /* The sqrt of the maximum nodes. Pre-computed.*/
  public SQRT_MAXNODES: number = Math.round(Math.sqrt(this.MAXNODES));
  public SQRT_MAXNODES_M1: number = this.SQRT_MAXNODES - 1;
  public INV_SQRT_M_1: number = 1.0 / (this.SQRT_MAXNODES - 1);
  /* The max amount of resistance the system can handle. */
  public R_MAX: number = 1e9;
  /* The max amount of resistance the system can handle. */
  public INV_R_MAX: number = 1.0 / this.R_MAX;
...
operating point analysis */
  ...
  constructor() {
    ...
    if (this.WORKSPACE_PERFECT_SQUARE) {
      ...
      this.WORKSPACE_RATIO_X =
        (view_port.view_height * this.WORKSPACE_RATIO_Y) / view_port.view_width;
    }
    /* Resistance for near zero voltage drop elements, like ammeters. */
