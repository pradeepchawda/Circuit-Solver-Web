...
class ElectricalNode {
  ...
  constructor(x, y, id) {
    ...
    this.node_fill_paint_alt.set_paint_style(
      this.node_fill_paint_alt.style.FILL
    );
    this.node_fill_paint_alt.set_paint_cap(this.node_fill_paint_alt.cap.ROUND);
    this.node_fill_paint_alt.set_paint_join(
      this.node_fill_paint_alt.join.MITER
    );
    ...
    this.node_fill_paint_alt.set_paint_align(
      this.node_fill_paint_alt.align.CENTER
    );
    /* This is used to keep a unique tag of the node in question. */
    this.id = id;
    /* This is used to re-map the nodes when doing simulation. */
    ...
    this.references = [];
    /* Mapping the ID to a row and column. This is useful to know where the node must go upon resizing. */
    this.column = Math.floor(
      this.id / Math.round(global.settings.SQRT_MAXNODES)
    );
    ...
    this.bounds = new RectF(
      this.location.x - node_space_x,
      this.location.y - node_space_y,
      this.location.x + node_space_x,
      this.location.y + node_space_y
    );
  }
  ...
  resize(n_x, n_y, m_n_x, m_n_y) {
    if (global.SIGNAL_BUILD_ELEMENT) {
      ...
      if (!global.MOBILE_MODE) {
        ...
        this.bounds.set_bounds(
          this.location.x - m_n_x,
          this.location.y - m_n_y,
          this.location.x + m_n_x,
          this.location.y + m_n_y
        );
      }
  ...
  draw(canvas: GraphicsEngine) {
    if (this.references.length > 0) {
      if (global.DEVELOPER_MODE) {
        canvas.draw_circle(
          this.location.x,
          this.location.y,
          2 * global.CANVAS_STROKE_WIDTH_3,
          this.node_line_paint
        );
        canvas.draw_text(
          this.debug_info(),
          this.location.x,
        ...
        if (global.WIRE_BUILDER['step'] > 0) {
          if (
            ...
            if (
              global.WIRE_BUILDER['n1'] != this.id &&
              this.draw_node_builder_helper()
            ) {
              global.NODE_LINE_BUFFER[global.NODE_LINE_BUFFER_INDEX++] = Array(
                this.bounds.left,
                this.bounds.top,
                this.bounds.right,
                this.bounds.top
