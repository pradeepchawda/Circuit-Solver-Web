 ...
 *
 * Purpose           : A class to handle the voltmeter element. It will automatically generate
 ...
 *
 ***********************************************************************/
var VoltMeter = /** @class */ (function () {
    function VoltMeter(type, id, n1, n2) {
      ...
      or overlapped)*/
        ...
        /* Inititalize the element2 class that will hold the basic data about our component */
        this.elm = new Element2(id, type, global.copy(global.PROPERTY_VOLTMETER));
    ...
    VoltMeter.prototype.refresh_bounds = function () {
        if (this.elm.consistent()) {
            this.p1 = new PointF(0, 0);
            this.p2 = new PointF(0, 0);
            /* Create some points to hold the node locations, this will be used for drawing components */
    ...
    VoltMeter.prototype.push_reference = function (ref) {
        this.wire_reference.push(ref);
    };
    VoltMeter.prototype.stamp = function () { };
    /* Vertex handling (for rotation) */
    VoltMeter.prototype.get_vertices = function () {
        var vertices = [];
        var p1 = [];
        var p2 = [];
    ...
    VoltMeter.prototype.release_wires = function () {
    ...
    VoltMeter.prototype.release_nodes = function () {
        if (this.elm.consistent()) {
    ...
    VoltMeter.prototype.capture_nodes = function () {
        var vertices = this.get_vertices();
        this.elm.map_node2(vertices[0], vertices[1], vertices[2], vertices[3]);
    ...
    VoltMeter.prototype.mouse_down = function () {
        if (global.FLAG_IDLE &&
    ...
    VoltMeter.prototype.handle_wire_builder = function (n, anchor) {
    ...
    VoltMeter.prototype.move_element = function (dx, dy) {
    ...
    VoltMeter.prototype.mouse_up = function () {
        if (global.FLAG_IDLE) {
            if (global.focused &&
                global.focused_id === this.elm.id &&
                global.focused_type === this.elm.type) {
    ...
    VoltMeter.prototype.select = function () {
        if (global.WIRE_BUILDER['step'] != 0) {
            wire_manager.reset_wire_builder();
        }
    ...
    VoltMeter.prototype.remove_focus = function () {
        if (global.focused &&
            global.focused_id === this.elm.id &&
            global.focused_type === this.elm.type) {
            global.focused_id = global.NULL;
    ...
    VoltMeter.prototype.remove_selection = function () {
        if (global.selected_id === this.elm.id &&
            global.selected_type === this.elm.type) {
            global.selected_id = global.NULL;
            global.selected_type = -1;
    ...
    VoltMeter.prototype.wire_reference_maintenance = function () {
    ...
    VoltMeter.prototype.unanchor_wires = function () {
        if (this.wire_reference.length > 0) {
            var vertices = this.get_vertices();
            var id = -1;
            for (var i = this.wire_reference.length - 1; i > -1; i--) {
    ...
    VoltMeter.prototype.anchor_wires = function () {
        if (this.wire_reference.length > 0) {
            var vertices = this.get_vertices();
            var id = -1;
            for (var i = this.wire_reference.length - 1; i > -1; i--) {
    ...
    VoltMeter.prototype.set_flip = function (flip) {
        this.BUILD_ELEMENT = true;
        wire_manager.reset_wire_builder();
        this.unanchor_wires();
        this.push_history();
    ...
    VoltMeter.prototype.set_rotation = function (rotation) {
        this.BUILD_ELEMENT = true;
        wire_manager.reset_wire_builder();
        this.unanchor_wires();
        this.push_history();
    ...
    VoltMeter.prototype.push_history = function () {
        if (this.INITIALIZED) {
            global.HISTORY_MANAGER['packet'].push(engine_functions.history_snapshot());
        }
    };
    /* Generate the SVG for the component. */
    VoltMeter.prototype.build_element = function () {
    ...
    VoltMeter.prototype.resize = function () {
        if (this.BUILD_ELEMENT || global.SIGNAL_BUILD_ELEMENT) {
    ...
    VoltMeter.prototype.refactor = function () {
    ...
    VoltMeter.prototype.update = function () { };
    VoltMeter.prototype.increment_rotation = function () {
        this.elm.rotation++;
    ...
    VoltMeter.prototype.increment_flip = function () { };
    VoltMeter.prototype.map_rotation = function () {
        if (this.elm.rotation === global.ROTATION_0 ||
    ...
    VoltMeter.prototype.reset_trace = function () {
        this.meter_trace.reset();
    };
    VoltMeter.prototype.reset_meter = function () {
        this.elm.properties['Voltage'] = 0;
    };
    VoltMeter.prototype.push_voltage = function (voltage) {
    ...
    VoltMeter.prototype.recolor = function () {
        if (global.selected) {
            if (global.selected_id === this.elm.id &&
                global.selected_type === this.elm.type) {
    ...
    VoltMeter.prototype.is_selected_element = function () {
        return (global.selected_id === this.elm.id &&
            global.selected_type === this.elm.type);
    };
    /* Draws the component */
    VoltMeter.prototype.draw_component = function (canvas) {
        ...
        if (global.PICTURE_REQUEST ||
            ...
            if ((this.ANGLE > 170 && this.ANGLE < 190) ||
                ...
                if (global.WORKSPACE_ZOOM_SCALE > 1.085 ||
                    ...
                    canvas.draw_text(global.ELEMENT_TAG_TEMPLATE.replace('{TAG}', this.elm.properties['tag']).replace('{ID}', String(this.elm.id)), this.c_x, this.bounds.bottom - this.bounds.get_height() * 0.025, this.text_paint);
                }
    ...
    VoltMeter.prototype.time_data = function () {
    ...
    VoltMeter.prototype.reset = function () {
        this.elm.properties['Voltage'] = 0;
    };
    return VoltMeter;
}());
