...
class ACSource {
  ...
  or overlapped)*/
    ...
    stamp() {
        if (this.elm.consistent()) {
            engine_functions.stamp_voltage(this.elm.n1, this.elm.n2, global.sine(2 * Math.PI * this.elm.properties['Frequency'] * global.simulation_time + global.to_radians(this.elm.properties['Phase'])) * this.elm.properties['Voltage'] +
    ...
    build_element() {
        if (this.BUILD_ELEMENT || global.SIGNAL_BUILD_ELEMENT) {
            ...
            this.sine_wave_p1.x = this.c_x - (cache_2 >> 1) * global.cosine(Math.PI + this.theta_m90);
            this.sine_wave_p1.y = this.c_y - (cache_3 >> 1) * global.sine(Math.PI + this.theta_m90);
            this.sine_wave_p2.x = this.c_x + (cache_2 >> 1) * global.cosine(Math.PI + this.theta_m90);
            this.sine_wave_p2.y = this.c_y + (cache_3 >> 1) * global.sine(Math.PI + this.theta_m90);
